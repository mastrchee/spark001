...
class OrderDetailTable extends Table {
  ...
  def getMappedRow(r : ResultSet) : Row = {
    return Row(
      ...
      if (r.getFloat("vat_value") > 0) (r.getFloat("price")/r.getFloat("total_price_without_delivery")).toFloat*r.getFloat("vat_value") else 0.0f, // vat_value per product (ternary check because of s**tty data)
